...
| 2. **Extract** | Vision API (Claude Sonnet) extracts structured data from images/PDFs | `POST /api/diagnostics/files/[id]/extract` |
...
**Architecture:**
```
Query → Python Agent → Query Analysis (Claude Haiku) → Embedding Generation
...
#### Chat Flow
```
User Message → API Route → Anthropic-backed agent → Stream Response
                     ↓
...
- Anthropic powers chat and OpenAI powers embeddings
