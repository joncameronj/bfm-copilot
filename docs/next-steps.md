...
1. **Comprehensive Health Check Endpoint** (`/api/admin/health`)
   - Monitors 11 services: Supabase DB, Supabase Auth, Python Agent, OpenAI API, + 7 internal APIs
...
3. **Chat** - Message send → Python agent → OpenAI API → response → save
...
7. **File Uploads** - OpenAI file API integration
8. **Analytics** - Usage tracking, dashboard stats

### External Dependencies

- **Supabase** - Authentication, PostgreSQL database, storage
- **OpenAI** - Chat completions, Vision API, embeddings, file handling
...
3. **If OpenAI API is Down**: Chat responses fail, suggest retry
...
4. **OpenAI API** - Key validation + API test
