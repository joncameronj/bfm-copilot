...
import { PatientProtocolSelector } from './PatientProtocolSelector'
...
export function ProtocolList({ viewMode = 'table' }: ProtocolListProps) {
  ...
  useEffect(() => {
    if (urlPatientId && urlPatientId !== filters.patientId) {
      setFilters(prev => ({ ...prev, patientId: urlPatientId }))
    }
  }, [urlPatientId])
  ...
  return (
    <div className="space-y-8">
      {/* Patient Selection & Generate Protocol */}
      <PatientProtocolSelector />

      {/* Existing Protocols Section */}
      <div>
        <h2 className="text-lg font-semibold text-neutral-900 dark:text-neutral-50 mb-4">Existing Protocols</h2>
            ...
            <div className="max-w-md mx-auto">
              ...
              <p className="text-neutral-600 dark:text-neutral-400 mb-6">
                {filters.search
                  ...
                  : 'Protocols are generated from diagnostic analyses. Select a patient above and upload diagnostics to generate protocols.'}
              </p>
            </div>
          </div>
        ) : viewMode === 'table' ? (
